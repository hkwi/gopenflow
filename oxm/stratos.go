...
func (self Stratos) FromOxm(buf []byte) string {
	...
	hdr := Oxm(buf).Header()
	length := hdr.Length() - 6
	value := buf[10 : 10+length]
	mask := []byte(nil)
	hasMask := hdr.HasMask()
	if hasMask {
		value = buf[10 : 10+length/2]
		mask = buf[10+length/2 : 10+length]
	}

	switch hdr.Field() {
	case STRATOS_OXM_FIELD_BASIC:
		switch binary.BigEndian.Uint16(buf[8:]) { // exp_type
...
func (self Stratos) ToOxm(txt string) (buf []byte, eatLen int, err error) {
	...
	case "radiotap_tsft":
		...
		buf = make([]byte, 18)
		binary.LittleEndian.PutUint64(buf[10:], v)
	...
	case "radiotap_lock_quality", "radiotap_tx_attenuation", "radiotap_db_tx_attenuation",
		...
		if err = parseInt(value, &v); err != nil {
			...
			buf = make([]byte, 12)
			binary.LittleEndian.PutUint16(buf[10:], v)
		} else if err = parseInt(mask, &m); err != nil {
			return
		} else {
			buf = make([]byte, 14)
			binary.LittleEndian.PutUint16(buf[10:], v)
			binary.LittleEndian.PutUint16(buf[12:], m)
		}
	case "radiotap_rate":
		...
		if n, err = fmt.Sscanf(value, "%fK", &kbps); err == nil && n == 1 {
			buf = make([]byte, 12)
			binary.LittleEndian.PutUint16(buf[10:], uint16(kbps/500.0))
		} else if n, err = fmt.Sscanf(value, "%fM", &kbps); err == nil && n == 1 {
			buf = make([]byte, 12)
			binary.LittleEndian.PutUint16(buf[10:], uint16(kbps*2))
		} else {
			if err == nil {
	...
	case "radiotap_channel":
		...
		if len(mask) == 0 {
			buf = make([]byte, 14)
			if !parse(value, buf[10:]) {
				return
			}
		} else {
			buf = make([]byte, 18)
			if !parse(value, buf[10:]) || !parse(mask, buf[14:]) {
				return
			}
		}
	case "radiotap_fhss":
		...
		if v, err = hex.DecodeString(value); err != nil {
			...
			buf = make([]byte, 10+length*2)
			copy(buf[10:], v)
			copy(buf[10+length:], m)
		}
	case "radiotap_mcs":
		...
		if len(mask) == 0 {
			...
			buf = make([]byte, 16)
			if !parse(value, buf[10:]) || !parse(mask, buf[13:]) {
				return
			}
		}
	case "radiotap_ampdu_status":
		...
		if len(mask) == 0 {
			...
			buf = make([]byte, 26)
			if !parse(value, buf[10:]) || !parse(mask, buf[18:]) {
				return
			}
		}
	case "radiotap_vht":
		...
		if len(mask) == 0 {
			...
			buf = make([]byte, 34)
			if !parse(value, buf[10:]) || !parse(mask, buf[22:]) {
				return
			}
		}
	}
	...
	switch {
	case strings.HasPrefix(label, "dot11_"):
		buf[2] = STRATOS_OXM_FIELD_BASIC << 1
	case strings.HasPrefix(label, "radiotap_"):
		buf[2] = STRATOS_OXM_FIELD_RADIOTAP << 1
	}
	if len(mask) > 0 {
		buf[2] |= 1
	}
	...
	binary.BigEndian.PutUint16(buf[8:], expType)
